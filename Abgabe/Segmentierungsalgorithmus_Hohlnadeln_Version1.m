%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SEMI-AUTOMATIC NEEDLE SEGMENTATION VERSION 1 
% for HDR-Brachytherapy of the Prostate in TRUS data at UKSH Kiel.
%
% This algorithm is to be used on already manually segmented TRUS data. 
% It automatically segments the implant needles in a TRUS dataset after 
% manual ROI selection. The manual segmentation result is plotted in
% orange, the automatic segmentation result in blue. The resulting coordi-
% nates of all segmented needles are stored in the array 
% automaticSegmentationPoints.
%
% Interaction required by the user:
% 1) The path to the TRUS dataset to load needs to be specified in line 57.
...
% Start timer.
%tic
%% Loading of Patient Dataset and Reference Segmentation.
% Load a patient dataset in DICOM format, getting the TRUS image data,
% DICOM information and the medical physicist's needle contour points
% stored in RTSTRUCT as reference segmentation.
% The user is required to insert the path to the DICOM data here:
[image,info,segmentation,infoRTSTRUCT,indexShift,vitessePoints] = showDICOMSegmentation...
    ('/home/jule/Studium/MSc_MedicalRadiationSciences/MRS/Masterarbeit_UKSH/Daten/17/',false);

% Store original image and segmentation before further modification.
originalImage = image;
originalSegmentation = segmentation;
...
if mod(size(image,3),2) == 0
    ...
    image = uint8(image);
    segmentation = zeros(size(originalSegmentation,1),size(originalSegmentation,2),...
        size(originalSegmentation,3)+1,3);
    segmentation(:,:,1:1:size(originalSegmentation,3),:) = originalSegmentation;
...
croppedSegmentation = imcrop3(segmentation,[xMin yMin zMin width height depth]);
...
% Plot the physicist's segmentation as a reference.
figure
imshow3Dfull(croppedSegmentation)

% Plot full size segmentation.
%figure
%imshow3Dfull(originalSegmentation)
...
% Get the coordinate position of the last slice in the DICOM patient-based
% coordinate system.
lastSliceCoordinate = info{1,size(info,2)}.ImagePositionPatient(3);
 
% For all segmentations in the reference segmentation.
for i = 1:size(vitessePoints,2)
    % Find the first contour point lying outside the TRUS image stack.
    firstIndexOutsideTRUS = find((vitessePoints{i}(3,:) < lastSliceCoordinate),1);
    
    % If the outermost point is on the outermost slice, use this point.
    if length(find((vitessePoints{i}(3,:) == lastSliceCoordinate),1)) == 1
        firstIndexOutsideTRUS = find((vitessePoints{i}(3,:) == lastSliceCoordinate),1);
    end
    % Calculate a reference point on the last slice using linear
    % interpolation.
    interpolationVector = vitessePoints{i}(:,firstIndexOutsideTRUS)-vitessePoints{i}(:,firstIndexOutsideTRUS-1);
    interpolationFactor = (lastSliceCoordinate-vitessePoints{i}(3,firstIndexOutsideTRUS-1))/interpolationVector(3,1);
    referencePointLastSlice = vitessePoints{i}(:,firstIndexOutsideTRUS-1)+interpolationFactor*interpolationVector;
    
    % For all points stored by Vitesse.
    for j = 1:firstIndexOutsideTRUS-1
        % Take all points from Vitesse that lie within the TRUS volume. 
        referencePoints{i}(:,j) = vitessePoints{i}(:,j);
    end
    % Add the interpolated point on the last slice.
    referencePoints{i}(:,j+1) = referencePointLastSlice;
end

% First match reference contours to labels from the automatic segmentation
% based on their position on the outmost axial slice.
matchingLabels = zeros(1,size(referencePoints,2));
indicesToRemove = {};
for i = 1:size(referencePoints,2)
    % Get the reference contour point on the outermost slice.
    referencePoint = [referencePoints{i}(1,size(referencePoints{i},2)),...
        referencePoints{i}(2,size(referencePoints{i},2))];
    distancesToReference = zeros(1,numberOfLabels);
    % For all labelled segmentations from the automatic segmentation.
    for j = 1:numberOfLabels
        % Get the labelled point on the outermost slice.
        labelPoint = [finalNeedlePointsCoordinates{1,j}(1,size(finalNeedlePointsCoordinates{1,j},2)),...
        finalNeedlePointsCoordinates{1,j}(2,size(finalNeedlePointsCoordinates{1,j},2))];
        % Calculate the distance of this labelled segmentation point to the
        % reference contour point on the same slice.
        distancesToReference(1,j) = norm(labelPoint-referencePoint);
    end
    % Find the label with the minimum distance to the reference contour
    % point.
    [minimum,label] = min(distancesToReference);
    % If there is a matching contour nearby, match label and reference
    % contour.
    if minimum < 5
        matchingLabels(1,i) = label;
    % Keep track for which contours no matching label could be found.
    else
        indicesToRemove{counter} = i;
        counter = counter+1;
    end
end

% Remove contours with no matching label from the automatic segmentation.
indicesToRemove = cell2mat(indicesToRemove);
for i = indicesToRemove
    matchingLabels(i) = [];
    referencePoints(i) = [];
end

% Find remaining unmatched reference points.
referencePointsToDelete = find(~matchingLabels);
% Remove zero entries in from the matchingLabels array.
matchingLabels(:,~any(matchingLabels,1)) = [];
% Remove the respective reference points.
for i = referencePointsToDelete
    referencePoints(i) = [];
end

% For all slices.
for i = 1:size(referencePoints,2)
    % Convert the reference points to voxel indices to match points for
    % comparison based on a mutual slice number.
    referencePointsVoxels{i} = coordinates2voxels(info,referencePoints{i});
end

% For all reference needle contours.
for i = 1:size(referencePointsVoxels,2)
    % Get the correct label index for comparison.
    labelForComparison = matchingLabels(1,i);
    
    % Determine the innermost slice this needle is detected on in the
    % automatic segmentation.
    tipSlice = finalNeedlePointsVoxels{labelForComparison}(3,1);
    counter = 1;
    % For all reference points of a single needle.
    for j = size(referencePointsVoxels{i},2):-1:1
        % Determine the slice number to perform the comparison on.
        sliceForComparison = referencePointsVoxels{i}(3,j);
        
        % If there is a point on that slice determined by automatic segmentation 
        % to compare to.
        if sliceForComparison >= tipSlice
            % Compare the needle positions in coordinates.
            pointsOnTheSameSlice = finalNeedlePointsCoordinates{labelForComparison}...
                (1:1:3,find(finalNeedlePointsVoxels{labelForComparison}(3,:)==sliceForComparison));
            % If there is more than one point on that slice. Use the outer
            % one.
            if size(pointsOnTheSameSlice,2) > 1
                outerPoint = pointsOnTheSameSlice(:,size(pointsOnTheSameSlice,2));
                clear pointsOnTheSameSlice
                pointsOnTheSameSlice = outerPoint;
            end
            automaticSegmentationPoints{i}(1:1:3,counter) = pointsOnTheSameSlice;
            counter = counter+1;
        else
            % Add the last point of the automatic segmentation if the
            % automatically defined needle ends earlier than the reference
            % contour.
            automaticSegmentationPoints{i}(1:1:3,counter) = finalNeedlePointsCoordinates{labelForComparison}(1:1:3,1);
        end     
    end
    % If the automatically segmented needle tip lies further in the
    % patient, add it to the array.
    if tipSlice < sliceForComparison
        automaticSegmentationPoints{i}(1:1:3,counter) = finalNeedlePointsCoordinates{labelForComparison}(1:1:3,1);
    end
    
    % Flip the array left to right to obtain points in the same order as the
    % reference points.
    automaticSegmentationPoints{i} = fliplr(automaticSegmentationPoints{i});
end

% For all segmentations.
for i = 1:size(automaticSegmentationPoints,2)
    % Convert the automatic segmentation points into voxels.
    automaticSegmentationPointsVoxels{i} = coordinates2voxels(info,automaticSegmentationPoints{i});
end

% Initialise arrays to store the distances between both segmentations for 
% points on the same slices and between needle tips.
distancesWithinSlices = {};
distancesTips = zeros(1,size(referencePoints,2));
% For all segmentations.
for i = 1:size(referencePoints,2)
    % Determine the number of reference points and points stored in the
    % automatic segmentation.
    numberOfReferencePoints = size(referencePoints{i},2);
    numberOfAutomaticSegmentationPoints = size(automaticSegmentationPoints{i},2);
    clear distanceOnSlice
    counter=1;
    % If both arrays have the same size.
    if numberOfReferencePoints >= numberOfAutomaticSegmentationPoints
        sizeComparison = numberOfReferencePoints - numberOfAutomaticSegmentationPoints;
        % For all points.
        for j = numberOfReferencePoints:-1:1+sizeComparison
            % Get the x and y coordinates of the reference point.
            referencePoint = [referencePoints{i}(1,j),referencePoints{i}(2,j)];
            % Store the reference point's slice position.
            referenceSlice = referencePointsVoxels{i}(3,j);
            % Get the x and y coordinates of the automatic segmentation point.
            automaticSegmentationPoint = [automaticSegmentationPoints{i}(1,j-sizeComparison),...
                automaticSegmentationPoints{i}(2,j-sizeComparison)];
            % Store the automatic segmentation point's slice position.
            automaticSegmentationSlice = automaticSegmentationPointsVoxels{i}(3,j-sizeComparison);
            % If both points lie on the same slice.
            if referenceSlice == automaticSegmentationSlice
                % Calculate the 2D Euclidean distance.
                distanceOnSlice{counter} = norm(automaticSegmentationPoint-referencePoint);
                counter = counter+1;
                % If at the tip location, 
                % calculate the 3D Euclidean distance between both points
                % and store it as both segmentations' tip distance.
                if j == 1+sizeComparison
                    automaticSegmentationPoint = [automaticSegmentationPoints{i}(1,1),...
                        automaticSegmentationPoints{i}(2,1),automaticSegmentationPoints{i}(3,1)];
                    referencePoint = [referencePoints{i}(1,1),referencePoints{i}(2,1),referencePoints{i}(3,1)];
                    tipDistance = norm(automaticSegmentationPoint-referencePoint);
                end
            elseif automaticSegmentationSlice > referenceSlice
                % If on the automatic segmentation tip slice, calculate the 3D Euclidean distance 
                % between both points and store it as both segmentations' tip distance.
                automaticSegmentationPoint = [automaticSegmentationPoints{i}(1,1),...
                    automaticSegmentationPoints{i}(2,1),automaticSegmentationPoints{i}(3,1)];
                % Get the reference tip point.
                referencePoint = [referencePoints{i}(1,1),referencePoints{i}(2,1),referencePoints{i}(3,1)];
                tipDistance = norm(automaticSegmentationPoint-referencePoint);
            end
        end
        % If the automatic segmentation extends further into the patient
        % than the reference segmentation.
    elseif numberOfAutomaticSegmentationPoints > numberOfReferencePoints
        % For all points in the automatic segmentation array.
        for k = numberOfAutomaticSegmentationPoints:-1:1
            if k > 1
                % Calculate the 2D Euclidean distances for points on the same
                % slices.
                referencePoint = [referencePoints{i}(1,k-1),referencePoints{i}(2,k-1)];
                automaticSegmentationPoint = [automaticSegmentationPoints{i}(1,k),automaticSegmentationPoints{i}(2,k)];
                distanceOnSlice{counter} = norm(automaticSegmentationPoint-referencePoint);
                counter = counter+1;
            elseif k == 1
                % Caluclate the 3D Euclidean distance for tip points.
                referencePoint = [referencePoints{i}(1,k),referencePoints{i}(2,k),referencePoints{i}(3,k)];
                automaticSegmentationPoint = [automaticSegmentationPoints{i}(1,k),...
                    automaticSegmentationPoints{i}(2,k),automaticSegmentationPoints{i}(3,k)];
                tipDistance = norm(automaticSegmentationPoint-referencePoint);
            end
        end  
    end
    % Correct the array order.
    flippedDistancesWithinSlices = fliplr(cell2mat(distanceOnSlice));
    % Store the 2D Euclidean distances between points on the same slice in coordinates.
    distancesWithinSlices{i} = flippedDistancesWithinSlices;
    % Store the 3D Euclidean distances between tip points in coordinates.
    distancesTips(1,i) = tipDistance;
end

% Create an array to store the angular differences in.
angularDifferences = NaN(1,10,size(referencePoints,2));

% Evalution of the angular difference in needle axis orientation.
% For all segmentations.
for i = 1:size(referencePoints,2)
    counter = 1;
    % Compare the numbers of points in both arrays.
    sizeComparison = size(referencePoints{i},2)-size(automaticSegmentationPoints{i},2);
    % If both arrays have the same number of points, determine vectors for
    % all adjacent point pairs for angular comparison.
    if sizeComparison == 0
        for j = size(referencePoints{i},2):-1:2
            vectorParallelReference = referencePoints{i}(:,j)-referencePoints{i}(:,j-1);
            vectorParallelAutomatic = automaticSegmentationPoints{i}(:,j)-...
                    automaticSegmentationPoints{i}(:,j-1);
            normVectorParallelReference = norm(vectorParallelReference);
            normVectorParallelAutomatic = norm(vectorParallelAutomatic);

            % Calculate the angular deviation of the needle axis sections in
            % degrees.
            angularDifferences(1,counter,i) = acosd(dot(vectorParallelAutomatic,vectorParallelReference)/...
                (normVectorParallelAutomatic*normVectorParallelReference));
            counter = counter+1;
        end
    elseif sizeComparison ~= 0
        % If there are fewer reference points.
        if sizeComparison < 0
            limit = size(referencePoints{i},2);
        % If there are fewer automatic segmentation points.
        elseif sizeComparison > 0
            limit = size(automaticSegmentationPoints{i},2);
        end
        for j = 0:1:limit-2
            vectorParallelReference = referencePoints{i}(:,end-j)-referencePoints{i}(:,end-j-1);
            vectorParallelAutomatic = automaticSegmentationPoints{i}(:,end-j)-...
                    automaticSegmentationPoints{i}(:,end-j-1);
            normVectorParallelReference = norm(vectorParallelReference);
            normVectorParallelAutomatic = norm(vectorParallelAutomatic);
...
% Evalution of the tip deviation directionality. 
% Initialise arrays to store the results.
tipDifferenceParallel = zeros(1,size(referencePoints,2));
tipDifferenceOrthogonal1 = zeros(1,size(referencePoints,2));
tipDifferenceOrthogonal2 = zeros(1,size(referencePoints,2));
tipDifferenceOrthogonal = zeros(1,size(referencePoints,2));

% For all segmentations.
for i = 1:size(referencePoints,2)
    % Determine the difference between both tip points.
    pointDifference = automaticSegmentationPoints{i}(:,1)-referencePoints{i}(:,1);
    
    % Calculate the unit vector along the innermost needle segment of the
    % reference data.
    vectorParallelReference = referencePoints{i}(:,1)-referencePoints{i}(:,2);
    vectorParallelReference = vectorParallelReference/norm(vectorParallelReference);
    % Determine both orthogonal unit vectors.
    vectorOrthogonalReference = null(vectorParallelReference(:).');
    vectorOrthogonalReference1 = vectorOrthogonalReference(:,1);
    vectorOrthogonalReference2 = vectorOrthogonalReference(:,2);
    
    % Calculate the tip detection array along the needle axis and
    % orthogonal to it.
    tipDifferenceParallel(1,i) = dot(pointDifference,vectorParallelReference);
    tipDifferenceOrthogonal1(1,i) = dot(pointDifference,vectorOrthogonalReference1);
    tipDifferenceOrthogonal2(1,i) = dot(pointDifference,vectorOrthogonalReference2);
    tipDifferenceOrthogonal(1,i) = sqrt(tipDifferenceOrthogonal1(1,i)^2+tipDifferenceOrthogonal2(1,i)^2);
end
        
%% Statistics for the Whole Dataset.
% Needle 2D distance evaluation for all needles of a dataset in mm.
all2DEuclideanDistances = cell2mat(distancesWithinSlices);

% Determine the number of point pairs compared for all segmentations.
for i = 1:size(distancesWithinSlices,2)
    numberOfPairs2DComparison(1,i) = size(distancesWithinSlices{i},2);
end

% Calculate the total number of pairs used for needle axis comparison in
% this dataset.
totalNumberOfPairs2DComparison = sum(numberOfPairs2DComparison);

% Calculate the mean and standard deviation of the  number of point pairs 
% for comparison per needle.
meanNumberOfPointPairsForComparisonPerNeedle = mean(numberOfPairs2DComparison);
stdNumberOfPointPairsForComparisonPerNeedle = std(numberOfPairs2DComparison);

% Calculate mean, maximum, standard deviation and standard error of the
% mean for the 2D Euclidean distances.
meanDistancesWithinSlices = mean(all2DEuclideanDistances);
[maxDistancesWithinSlices,~] = max(all2DEuclideanDistances);
stdDistancesWithinSlices = std(all2DEuclideanDistances);
semDistancesWithinSlices = stdDistancesWithinSlices/sqrt(totalNumberOfPairs2DComparison);

% Determine the number of axis segments compared for all segmentations.
for i = 1:size(angularDifferences,3)
    numberOfSegmentsForComparison(1,i) = sum(~isnan(angularDifferences(1,:,i)));
end

% Calculate the total number of pairs used for needle axis comparison in
% this dataset.
totalNumberOfSegmentsForComparison = sum(numberOfSegmentsForComparison);

% Calculate the mean and standard deviation of the needle axis segments
% used per needle.
meanNumberOfSegmentsForComparisonPerNeedle = mean(numberOfSegmentsForComparison,'omitnan');
stdNumberOfSegmentsForComparisonPerNeedle = std(numberOfSegmentsForComparison,'omitnan');

% Calculate the mean and the standard deviation of the needle axis angular 
% difference in degrees.
meanAngularDifference = mean(angularDifferences,'all','omitnan');
stdAngularDifference = std(angularDifferences,0,'all','omitnan');
[maxAngularDifference,~] = max(max(angularDifferences));
semAngularDifference = stdAngularDifference/sqrt(totalNumberOfSegmentsForComparison);

% Calculate the mean and the standard deviation of the 3D tip distance
% for all needles of a dataset.
meanDistanceTips = mean(distancesTips);
stdDistanceTips = std(distancesTips);

% Calculate the mean and the standard deviation of the tip directional errors.
meanParallelTipError = mean(tipDifferenceParallel);
stdParallelTipError = std(tipDifferenceParallel);
meanOrthogonalTipError = mean(tipDifferenceOrthogonal);
stdOrthogonalTipError = std(tipDifferenceOrthogonal);

%% DICOM-Export.
% % Copy the original DICOM info of the RTSTRUCT file for modification.
% infoRTSTRUCTModified = infoRTSTRUCT;
% 
% % For all needles detected.
% for i = 1:size(automaticSegmentationPoints,2)
%     counter = 1;
%     % For all segmentation points.
%     for j = 1:size(automaticSegmentationPoints{i},2)
%         % Store the points in the same format as they are stored in the
%         % DICOM RTSTRUCT file.
%         automaticSegmentationDicomFormat{i}(counter:1:counter+2,1) = automaticSegmentationPoints{i}(:,j);
%         counter = counter+3;
%     end
%     % Write the automatical segmentation into infoRTSTRUCTModified. 
%     eval(sprintf('infoRTSTRUCTModified.ROIContourSequence.Item_%d.ContourSequence.Item_1.ContourData = automaticSegmentationDicomFormat{i}', i+indexShift));
% end
% 
% % Determine the number of contours that were correctly detected by the algorithm 
% % and the number of contours of the original segmentation.
% numberOfContoursToCompare = size(automaticSegmentationPoints,2);
% numberOfOriginalContours = numel(fieldnames(infoRTSTRUCT.ROIContourSequence))-indexShift;
% % If not all needles were detected.
% if numberOfContoursToCompare < numberOfOriginalContours
%     % Find surplus contours.
%     surplusContours = ~ismember(1:numberOfOriginalContours,1:numberOfContoursToCompare).*(1:numberOfOriginalContours);
%     surplusContours(:,~any(surplusContours,1)) = [];
%     % Delete surplus contour data from the original file that are not overwritten.
%     for i = surplusContours
%         % Write into infoRTSTRUCTModified. 
%         eval(sprintf('infoRTSTRUCTModified.ROIContourSequence.Item_%d.ContourSequence.Item_1.ContourData = []', i+indexShift));
%     end
% end
% 
% % Write new DICOM RTSTRUCT file from infoRTSTRUCTModified
% dicomwrite([],'SS001.dcm',infoRTSTRUCTModified,'CreateMode','copy'); 
...
function [image,info,superposition,infoRTSTRUCT,indexCorrection,vitessePoints] = showDICOMSegmentation(path,plot)
% SHOWDICOMSEGMENTATION - Plot DICOM RTSTRUCT segmentation and extract
% contour information.
% The function loads DICOM TRUS images and the associated RTSTRUCT applicator 
% segmentation data, calculates the superimposed data and plots it if plot
% is set to true. The needle contour points stored in the RTSTRUCT files
% are interpolated linearly to define the needles in all slices.
%
% Inputs:
%    path - path to folder containing DICOM image and RTSTRUCT
%    plot - choose to plot scoordinates2voxelsuperimposed data (boolean)
%
% Outputs:
%    image - image in 3D, uint8
%    info - image DICOM information
%    superposition - superposition of image and segmentation, 4D RGB, uint8
%    infoRTSTRUCT - RTSTRUCT DICOM information
%    indexCorrection - index shift to set first applicator ROI to index 1
%    vitessePoints - contour points as defined by Vitesse
%
...
%------------- CODE --------------
    ...
    % Get RTSTRUCT DICOM info.
    infoRTSTRUCT = dicominfo([path 'SS001.dcm']);

    % Load TRUS dataset.
    [image,info] = loadTRUS(path);
    image = squeeze(image);

    % Get all contours from RTSTRUCT.
    contours = dicomContours(infoRTSTRUCT);

    % Plot all ROIs (needles and OARs) in RTSTRUCT in 3D volume.
    figure
    plotContour(contours);

    % Get DICOM contour names of the first few structures.
    names = {infoRTSTRUCT.StructureSetROISequence.Item_1.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_2.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_3.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_4.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_5.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_6.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_7.ROIName;
            infoRTSTRUCT.StructureSetROISequence.Item_8.ROIName};

    % Find first applicator data.
    for i=1:size(names,1)
        if strcmp(names{i},'Applicator01') == 1
            firstApplicator = i;
            break;
        end
    end
    
    % Index correction to avoid OAR structures.
    indexCorrection = firstApplicator-1;

    % Arrays for all contour points in coordinates and voxels.
    contourPointsCoordinates = cell(1,size(contours.ROIs,1)-indexCorrection);
    contourPointsVoxels = cell(1,size(contours.ROIs,1)-indexCorrection);
   
    % For every applicator.
    for i = firstApplicator:size(contours.ROIs,1)
        counter = 1;
        % For every line between points.
        for k = 1:size(contours.ROIs.ContourData{i}{1},1)-1
            % Get two adjacent points.
            p1 = contours.ROIs.ContourData{i}{1}(k,:);
            p2 = contours.ROIs.ContourData{i}{1}(k+1,:);
            
            % Save DICOM RTSTRUCT contour points in coordinates.
            if k == 1
                vitessePoints{i-indexCorrection}(:,counter) = p1;
                counter = counter+1;
                vitessePoints{i-indexCorrection}(:,counter) = p2;
                counter = counter+1;
            else
                vitessePoints{i-indexCorrection}(:,counter) = p2;
                counter = counter+1;
            ...
            % Get evenly spaced z-coordinates with 2.5 mm spacing.
            zTemp = p1(3):-2.5:p2(3);
            % Add endpoint to zValue array.
            zValues = zeros(1,length(zTemp)+1);
            zValues(1:length(zValues)-1) = zTemp;
            zValues(length(zValues)) = p2(3);

            % Determine stepsizes to calculate missing x and y coordinates for
            % points on line.
            stepsizes = (zValues-p1(3))/(p2(3)-p1(3));
            % Get points on line: PointsOnLine = p1 + stepsize(p2-p1).
            xValues = p1(1)+stepsizes*(p2(1)-p1(1));
            yValues = p1(2)+stepsizes*(p2(2)-p1(2));

            % Store needle coordinates in patient-based coordinate system.
            contourPointsCoordinates{i-indexCorrection}{k} = ([xValues;yValues;zValues]);

            % Convert coordinates into voxels.
            contourPointsVoxels{i-indexCorrection}{k} = coordinates2voxels(...
                info,contourPointsCoordinates{i-indexCorrection}{k});
        end
    end

    % Create 3D space with needle ROIs.
    needleROIs = zeros(size(image,1,2,3));
    % For all applicators.
    for  i = 1:size(contourPointsVoxels,2)
        % For all lines representing the same applicator.
        for j = 1:size(contourPointsVoxels{i},2)
            % For all points representing one line.
            for k = 1:size(contourPointsVoxels{i}{j},2)
                needleROIs(contourPointsVoxels{i}{j}(1,k),...
                    contourPointsVoxels{i}{j}(2,k),contourPointsVoxels{i}{j}(3,k)) = i;
            end
        end
    end

    % Remove slices where applicator ROIs extend beyond the TRUS z-range.
    needleROIs = uint8(needleROIs(:,:,1:1:size(info,2)));

    % Convert TRUS greyscale image to RGB uint8.
    rgbImage = cat(4,image,image,image);

    % Convert needleROIs to RGB uint8 colouring needles orange.
    orangeNeedles = needleROIs(:,:,:);
    orangeNeedles(orangeNeedles>0) = 255;
    
    % Zoom spots using 2D 3 x 3 mean filter.
    meanInput = 1/3*ones(3,1);
    meanFilter = meanInput*meanInput';
    for i = 1:size(orangeNeedles,3)
        orangeNeedles(:,:,i) = filter2(meanFilter,orangeNeedles(:,:,i));
    end
    orangeNeedles = double(orangeNeedles)*double(255/double((max(max(max(orangeNeedles))))));
    rgbNeedles = zeros([size(orangeNeedles) 3]);
    rgbNeedles(:,:,:,1) = orangeNeedles;
    rgbNeedles(:,:,:,2) = orangeNeedles.*165/255;
    rgbNeedles = uint8(rgbNeedles);

    % Superimpose TRUS and needle ROIs.
    alpha = 0.4;
    superposition = alpha*rgbImage+(1-alpha)*rgbNeedles;
    
    if plot == true
        % Plot superimposed images as image stack.
        figure
        imshow3Dfull(superposition)
    end
end
